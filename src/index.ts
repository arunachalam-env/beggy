...
app.post('/api/readings', mutationLimiter, (req: Request, res: Response) => {
  try {
...
app.get('/api/chain/latest', (req: Request, res: Response) => {
...
app.get('/api/chain/verify', (_req: Request, res: Response) => {
  res.json(verifyEntireChain());
});

/** POST /api/keys/register — dev-only keypair generation */
app.post('/api/keys/register', mutationLimiter, (_req: Request, res: Response) => {
  if (!isDev) {
